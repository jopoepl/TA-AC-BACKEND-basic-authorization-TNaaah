...
router.get(`/all`, Auth.loggedInAdmin,  (req, res, next) => {
  User.find({}, "fname email userType", (err, users) => {
    if(err) next(err);
...
/* GET -- BLOCKING A USER */
router.post(`/:id/edit/type`, (req,res,next) => {
  let id = req.params.id
  let type = req.body.userType
  console.log("usetype edit", type)
    User.findOneAndUpdate({_id: id}, {userType: type}, (err, updatedUser) => {
      if(err) next(err);
